...
export async function GET(request: Request) {
  ...
  if (error) {
    ...
    const message = /AADSTS65001|consent/i.test(description ?? "")
      ...
      : description || error;
    return NextResponse.redirect(new URL(`/settings?oauthError=${encodeURIComponent(message)}`, origin));
  }
  const code = url.searchParams.get("code");
  const state = url.searchParams.get("state");
  const session = await getSession();
  if (!code || !state || state !== session.oauthState) {
    return NextResponse.redirect(new URL("/settings?oauthError=Invalid+OAuth+state.+Try+connecting+again.", origin));
  }
  try {
    ...
    return NextResponse.redirect(
      new URL(`/settings?oauthError=${encodeURIComponent(publicError(err))}`, origin),
    );
  }
}
